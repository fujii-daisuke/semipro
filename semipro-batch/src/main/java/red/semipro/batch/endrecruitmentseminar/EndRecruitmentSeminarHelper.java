...
public class EndRecruitmentSeminarHelper {
    ...
    public void execute(@Nonnull final LocalDateTime executionDateTime) {

        List<Seminar> seminarList =
            seminarRepository.findAllEndOfRecruitment(OpeningStatus.OPENING, executionDateTime);

        log.info("募集終了件数は " + seminarList.size() + "件です");

        seminarList.forEach(seminar -> {
            try {
                establishSeminarService.establish(seminar.getId());
